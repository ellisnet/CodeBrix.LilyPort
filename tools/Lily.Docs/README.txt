...
PICTURES -- SVG IN, VECTORS OUT, AT THE PICTURE'S OWN PHYSICAL SIZE

Html2Pdf places the SVG itself (decision D51, dissolved), so Lily.Docs hands the
same file to both outputs and owns no rasterizer. ⚠ D51 WAS AMENDED 2026-08-26:
from Texinfo2Pdf 1.0.238.626 (Html2Pdf 1.0.238.580) the picture is written into
the PDF as VECTOR content -- the engraving's paths, strokes and dashes as PDF
operators, its lyrics and markup as real embedded-font text (selectable and
searchable) -- and no image is embedded for it at all. Options.Html.SvgPlacement
= Vector is the package default AND is set explicitly in ApplyPdfOptions, then read
back into the baseline as SVG_PLACEMENT, so a package that changed its default
would move a row rather than quietly turn 2,555 pictures into bitmaps.

  * The placed size is the SVG's OWN size in millimetres, now EXACTLY: the root's
    declared width/height are honoured to the point (139.04mm = 394.13pt), where
    the raster route rounded them to whole CSS pixels first (0.04% large). That
    rounding is what moved several page counts at this bump -- see BASELINES.
  * Options.Html.SvgRasterScale (2.0, still the shipped default and still
    recorded) now governs ONLY a part of a picture PDF cannot express -- an image
    filter, a repeating gradient, a translucent-stop gradient, a pattern fill --
    which the package rasterizes on its own and reports as "image.svg.rasterized".
    The engraving dialect contains none of those, and the baselines say so: zero
    such warnings across all nine manuals.
  * Raster placement remains a supported option of the package for other
    consumers. These manuals never ask for it.

  HISTORY, wave LD4 (raster route, through 2026-08-25): the placed size was the
  CSS-pixel bounds at 0.75pt/px; 192 ppi = 96 CSS dpi x SvgRasterScale; 2,519 of
  2,554 embedded rasters sat at exactly 192 ppi, ONE wider-than-column engraving at
  252 ppi, and TWO @sourceimage photographs at their natural 134/135 ppi.
...
WHAT THE OUTPUT DIRECTORY NO LONGER CARRIES

Nothing native. Through 2026-08-25 SVG rasterization was Skia, and SkiaSharp and
HarfBuzzSharp arrived through the chain with native assets for every RID they
ship (MEASURED 550 MB, 40 files across 17 runtime identifiers, of which this
machine used linux-x64's 15 MB) -- the price of decision D51, paid by a repo tool
that ships nothing. Since the 2026-08-26 bump the chain is fully managed and that
payload is gone; the SkiaSharp.NativeAssets.Linux reference Lily.Docs.csproj
carried for it is removed, and decision D52 is satisfied by construction.
...
--------------------------------------------------------------------------------
    ...
    <manual>-pdf.tsv       PAGES, PDF_WARNINGS and PDF_ITEMS; the page size the
                           ...
                           how the pictures were placed (SVG_PLACEMENT, Vector
                           since 2026-08-26 -- set explicitly and read back);
...
engravings from 12 to 1. ⚠ THE 2026-08-26 VECTOR BUMP (Texinfo2Pdf 1.0.238.626)
MOVED NONE OF THEM: all nine PAGES rows, both warning counts, every DROP row and
every snippet count re-froze identical, and the only new row is SVG_PLACEMENT.
What did move is the files -- notation's PDF 33.8 MB -> 12.5 MB, learning's 5.6
-> 2.8 MB -- because 2,555 pictures are paths and embedded-font text now instead
of 192 ppi bitmaps. If you change a baseline, change this table in the same
