...
public sealed class ManualRenderer
{
    ...
    /// <remarks>
    /// Everything else is left at the package's defaults DELIBERATELY, and two of those
    /// defaults are load-bearing enough to be recorded in the baseline rather than
    ...
    private static PdfRenderSettings ApplyPdfOptions(HtmlRenderOptions options)
    {
        ...
        // ── Decision D51, AMENDED 2026-08-26: the pictures are VECTORS ───────────────
        //
        // D51 dissolved into "Html2Pdf places the SVG itself", and the route it took
        // in 1.0.232.x was a raster at 96 CSS dpi x SvgRasterScale. From Html2Pdf
        // 1.0.238.580 (Texinfo2Pdf 1.0.238.626) the package writes the SVG's display list
        // into the page as PDF operators — paths, strokes, dashes, and the lyrics and
        // markup as real embedded-font text — and Vector is its default. It is set here
        // EXPLICITLY anyway, so that a package that changed its default would show up as
        // a moved SVG_PLACEMENT row in the baseline, not as 2,555 pictures quietly
        // turning into bitmaps. Raster remains a supported placement for other
...
public sealed class PdfRenderSettings
{
    ...
    /// <summary>
    /// How the PDF stage placed the engraved pictures: "Vector" (PDF paths and embedded-font
    /// text, the package default and what these manuals set explicitly) or "Raster".
    /// </summary>
    /// <remarks>
    /// Decision D51 as AMENDED 2026-08-26. Read back off the live options after
    /// <c>ApplyPdfOptions</c> set it and frozen as SVG_PLACEMENT, so a package that changed
    /// its default, or a render that asked for the other mode, moves a baseline row instead
    /// of silently changing what two and a half thousand pictures are made of.
    /// </remarks>
    public string SvgPlacement { get; }

    /// <summary>
    /// The scale at which the PDF stage rasterizes a part of a picture it cannot express
    /// as vectors (image filters, Porter-Duff compositing, repeating gradients …), relative
    /// to the picture's CSS-pixel size. Under Vector placement nothing in these manuals
    /// takes that path; under Raster placement it would be the whole picture's density.
    /// </summary>
    /// <remarks>
    /// ⚠ NOT A DPI, AND NOT OURS TO SET WELL OR BADLY. Through wave LD5 this was the whole
    /// of the picture-quality decision (every engraving was a 192 ppi raster). Since the
    /// vector route it governs only the fallback; it stays recorded because the package
    /// still owns it and a moved value would still mean something.
    /// </remarks>
