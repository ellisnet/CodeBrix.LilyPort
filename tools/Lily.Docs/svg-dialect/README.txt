...
1.0.232.60 placed the SVG itself, rasterizing it through Skia at 96 CSS dpi times
Options.Html.SvgRasterScale (2.0, so 192 ppi) and at the SVG's own physical
millimetres. ⚠ AMENDED 2026-08-26: from Html2Pdf 1.0.238.580 (Texinfo2Pdf
1.0.238.626) the picture is placed as PDF VECTOR content -- its paths, strokes and
dashes as operators and its text as embedded-font text -- through a fully managed
engine (CodeBrix.Imaging.Drawing.NoSkia), with no Skia and no native library; the
raster route survives only as an opt-in placement mode nobody here uses. Lily.Docs
still builds no rasterizer and hands the SAME file to both outputs. What survives the ruling is this specification: the renderer that now
