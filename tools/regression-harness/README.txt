...
================================================================================
  ...
  COMMITTED:
    ...
    reference-manifest.tsv      one line per reference output: sha256, byte size,
                                file name. Small, diffable, and enough to detect
                                that a reference has drifted.

    reference-status.txt        per-input outcome (section 7).
...
THIRD-PARTY-NOTICES.txt section 1.
...
================================================================================
  ...
  Comparison reports MISSING for everything
      CodeBrix.LilyPort has not produced output, or CANDIDATE_SVG_DIR is not the
      directory the port rendered into. Not an expected state any more: the port
      engraves the whole suite, and G1 has been green since 2026-08-18
      (g1-skip-list.tsv is the committed record).
...
RUNNING IT
  ...
  The clean's "entr(ies) that are neither .svg nor .midi remain ... is this the
  directory you meant?" NOTE excludes .midi as of 2026-08-27. It did not before, so a
  normal sweep of the regression corpus ended its clean with "66 non-.svg entr(ies)
  remain" every single time -- a permanent false alarm over the driver's own output.
  A warning that always fires teaches the reader to stop reading it, which is worse
  than no warning; the note now fires only for something genuinely foreign.
