...
/// <c>\paper</c> block, read at startup into <c>$defaultpaper</c>. This reconstructs
/// the subset the engraving path reads, in C#. The derivations are translated from
/// <c>scm/paper.scm</c>'s <c>layout-set-absolute-staff-size-in-module</c> and
/// <c>calc-line-thickness</c> rather than invented, so the numbers are upstream's.
/// </para>
/// <para>
/// //was previously: this block said "Track P has not been built, so this reconstructs
/// …" and called the class a STAND-IN that "stops being one the moment the parser can
/// read the real <c>\paper</c> block". Track P IS built and the parser DOES read the
/// real block — the claim was false, and the class did not retire when it came true.
/// It has two live jobs instead. First, <c>LilyPondInit</c> parents it in as a FALLBACK
/// when the init layer hands back a layout that cannot resolve <c>output-scale</c>,
/// where the alternative is every dimension silently coming out zero. Second, it is how
/// a scm-less fixture builds a layout at all: <c>PaperScoreTests</c>,
/// <c>StemEngraverTests</c> and <c>VerticalOrganizationEngraverTests</c> all start from
/// it. Whether the FALLBACK branch is still reachable is a separate question and is NOT
/// settled here: answering it needs the branch instrumented and a sweep run, which is
/// its own small piece of work.
/// </para>
/// <para>
/// Only the variables the engrave path actually reads are set — the page-layout,
/// titling and vertical-spacing variables are deliberately absent rather than guessed,
/// so a caller that needs one gets an honest "unset" instead of a plausible wrong
/// number.
/// </para>
/// </summary>
