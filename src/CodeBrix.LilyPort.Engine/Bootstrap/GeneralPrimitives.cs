...
public static class GeneralPrimitives
{
    ...
    private static void InstallOptions(Interpreter interpreter, ProgramOptions options)
    {
        ...
        // check_value_type calls a declared #:type PREDICATE, so the store needs a way
        // to call one. Installed here rather than reached for through the ambient
        // LilyPondScheme.Current, which is still null while lily.scm loads and sets
        // options. Upstream's ly_call answers a value and the caller asks scm_is_false,
        // so anything but #f satisfies the predicate.
        options.TypePredicateCheck = (predicate, value) =>
            Evaluator.IsTrue(interpreter.Evaluator.Apply(predicate, new[] { value }));
        ...
        interpreter.DefinePrimitive("ly:add-option", 3, -1, a =>
        {
            ...
            // An option declared with no #:type is a BOOLEAN option, and upstream says
            // so by storing the `boolean?' predicate as its type
            // (lily/program-option-scheme.cc:281-282, Guile_user::boolean_p) rather than
            // by leaving the property unset -- which is what makes check_value_type
            // refuse `(ly:set-option 'warning-as-error 'yes)'. //was previously: the
            // property was set only when #:type was given, so every un-typed option was
            // indistinguishable from an option nobody declared and nothing was checked.
            ...
            {
                type = interpreter.GuileModule
                    .Lookup(Symbol.Intern("boolean?"))?.GetValue();
            }

            if (type != null)
            {
                options.DeclareType(name, type);
                SetObjectProperty(
        ...
        // different mistakes, in that order and with the value type checked last. All
        // three live in AppendToOption, which is where CommandLineOptions reaches them
        // too, exactly as SetFromOptionName carries ly_set_option's own sequence.
        interpreter.DefinePrimitive("ly:append-to-option", 2, 2, a =>
        {
            options.AppendToOption(AsSymbolName(a[0]), a[1]);
            return Unspecified.Instance;
        });
