...
public sealed class TextFontMetric : FontMetric
{
    ...
    public string DescriptionString
    {
        get
        {
            ...
            // The FORMAT is not cosmetic: this string is the ONLY route the size takes to
            // the backend, which parses it back out with upstream's own regular expression
            // (output-svg.scm's pango-description-to-text) and divides by output-scale to
            // write font-size. See PangoSizeText for what the format IS and how it was
            // measured.
            text.Append(PangoSizeText(Size));
            return text.ToString();
        }
    }

    /// <summary>
    /// Formats a size the way <c>pango_font_description_to_string</c> does: the SHORTEST
    /// decimal string that rounds back to the same whole number of Pango units.
    /// </summary>
    /// <remarks>
    /// <para>
    /// //was previously: <c>Size.ToString ("0.000")</c>, under a comment claiming that
    /// <c>pango_font_description_to_string</c> "formats the description's size to three
    /// decimals" and that the format had been "MEASURED against the pinned oracle across
    /// 31 font sizes ... reproduces every one". Both halves were wrong, and PORT-COVERAGE
    /// had already flagged them as wrong: the corpus holds counterexamples the claim
    /// cannot express. Three decimals happens to be what Pango emits for MOST sizes,
    /// which is why 31 samples missed it.
    /// </para>
    /// <para>
    /// The rule here is MEASURED, by driving libpango rather than reading it —
    /// <c>tools/parity-probes/lilyport-probe-pango-size/pango_desc.py</c> builds a real
    /// <c>PangoFontDescription</c>, sets an integer size and reads what
    /// <c>to_string</c> writes. Over 29,353 sizes from 1/1024 pt to 195 pt, this rule
    /// reproduces libpango's output with ZERO mismatches, where three decimals misses 412
    /// of the first 17,555 and <c>%g</c> misses more. Note the round trip is through the
    /// PANGO-UNIT LATTICE, not through the double: 3.9794921875 prints as
    /// <c>3.9795</c> — four decimals — because <c>3.979</c> would land on a different
    /// unit, while 3.919921875 prints as <c>3.92</c> because two decimals suffice.
    /// </para>
    /// <para>
    /// The caveat the probe carries carries here too: the pinned oracle bundles Pango
    /// 1.57.0 and the probe drove 1.56.3. What rests on the probe is the RULE; what rests
    /// on the pinned oracle itself is the corpus, and the four sizes ruling R10 had to
    /// accept a bounded delta on are reproduced exactly by this rule.
    /// </para>
    /// </remarks>
    /// <param name="size">The size, already on the Pango lattice.</param>
    /// <returns>The size as Pango would write it.</returns>
    internal static string PangoSizeText(double size)
    {
        long units = (long)Math.Round(size * PangoScale, MidpointRounding.AwayFromZero);
        for (int digits = 0; digits <= 17; digits++)
        {
            string candidate = size.ToString(
                "F" + digits.ToString(CultureInfo.InvariantCulture), CultureInfo.InvariantCulture);
            if (candidate.Contains('.'))
            {
                candidate = candidate.TrimEnd('0').TrimEnd('.');
            }

            double parsed = double.Parse(candidate, CultureInfo.InvariantCulture);
            if ((long)Math.Round(parsed * PangoScale, MidpointRounding.AwayFromZero) == units)
            {
                return candidate;
            }
        }

        return size.ToString("R", CultureInfo.InvariantCulture);
    }
