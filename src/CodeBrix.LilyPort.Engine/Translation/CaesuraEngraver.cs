...
public class CaesuraEngraver : Engraver
{
    ...
    public override void StopTranslationTimestep()
    {
        ...
        if (_scripts.Count > 0)
        {
            ...
            //
            //was previously: a private two-line copy of Script_interface::script_priority_less,
            // taken before script-interface.cc was ported. The copy read the property with
            // TranslatorSchemeHelpers.ToLong, where upstream's comparator reads it with
            // from_scm<int>; ScriptInterface.ScriptPriorityLess is that comparator, so the
            // call below is both the dedup and the faithful read.
            //
            // List<T>.Sort is NOT stable and upstream's is std::stable_sort, so the original
            // index carries the tie -- which is exactly what a stable sort guarantees and
            // nothing more.
            ...
            keyed.Sort((a, b) =>
            {
                if (ScriptInterface.ScriptPriorityLess(a.Script, b.Script))
                {
                    return -1;
                }

                return ScriptInterface.ScriptPriorityLess(b.Script, a.Script)
                    ? 1
