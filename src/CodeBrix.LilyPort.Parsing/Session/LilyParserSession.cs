...
public sealed partial class LilyParserSession : IParserHost, ILexerHost
{
    ...
    /// <summary>
    /// Gets how many orphan scopes <see cref="RestoreToplevelScope"/> has had to discard
    /// over this session's life — scopes an earlier file opened and never closed because
    /// its parse died inside the block. A non-zero count means some file left the stack
    /// deeper than it found it; it is reported per run so the condition is visible rather
    /// than silently corrected.
    /// </summary>
    public int OrphanScopesDiscarded { get; private set; }
    ...
    // How deep the scope stack was when the snapshot was taken — one, the base scope,
    // right after the init layer loaded. See RestoreToplevelScope's ELEVENTH LEAK note.
    private int _toplevelScopeDepth = -1;
    ...
    public void SnapshotToplevelScope()
    {
        ...
        _toplevelScopeDepth = _scopes.Count;
    }
    ...
    /// </para>
    /// <para>
    /// THE ELEVENTH LEAK — the SCOPE STACK ITSELF, not just what is in the base scope.
    /// <see cref="AddScope"/> is called when a <c>\layout</c>, <c>\midi</c>,
    /// <c>\paper</c> or <c>\with</c> block opens and <see cref="RemoveScope"/> only
    /// when the grammar action that closes it runs. A file whose parse dies inside such
    /// a block never runs that action, so its scope stays on the stack — and because
    /// <see cref="SetIdentifier"/> writes to the INNERMOST scope, every later file's
    /// toplevel assignments then land in that orphan instead of the base scope, where
    /// this method cannot see them to remove them. Found in the Mutopia sweep: after 95
    /// files had accumulated an orphan scope, greensleeves_guitar.ly's
    /// <c>"\\&lt;" = #(define-event-function (pos) (markup?) ...)</c> outlived its own
    /// file and turned <c>\&lt;</c> into a markup-taking function for every later file,
    /// so the Corelli quartet and the Mendelssohn Octet parsed their hairpins as
    /// markup — 62 phantom errors on one, no score at all on the other. Run alone the
    /// same files are clean: the full-sweep-only trap again, and the reason the stack is
    /// trimmed here before anything else is restored.
    /// </para>
    /// </summary>
    public void RestoreToplevelScope()
    {
        ...
        // Trim FIRST: while an orphan scope is still on top, the base scope is not where
        // identifiers are being written and restoring it would fix nothing.
        while (_toplevelScopeDepth > 0 && _scopes.Count > _toplevelScopeDepth)
        {
            _scopes.RemoveAt(_scopes.Count - 1);
            OrphanScopesDiscarded++;
        }

        // The discarded module may still be what the interpreter calls current; re-point
        // it at the base scope before anything is restored into that scope.
        SetCurrentScope();
