...
public static class PythonRegex
{
    ...
    /// <summary>python's <c>re.split</c>: cuts the text at every match.</summary>
    /// <param name="pattern">The pattern.</param>
    /// <param name="text">The text to cut.</param>
    /// <param name="options">The flags.</param>
    /// <returns>The pieces, including the empty ones a match at either end leaves.</returns>
    /// <remarks>
    /// Added for the MusicXML importer, which reads a volta list this way. .NET's own
    /// <c>Regex.Split</c> would also return the CAPTURED groups between the pieces,
    /// which python does not do unless the pattern captures deliberately; the patterns
    /// here do not capture, so the two agree, and this wrapper exists to say so.
    /// </remarks>
    public static List<string> Split(
        string pattern, string text, RegexOptions options = RegexOptions.None)
        => new List<string>(Compile(pattern, options).Split(text ?? string.Empty));
