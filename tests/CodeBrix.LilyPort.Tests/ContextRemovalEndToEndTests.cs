...
public class ContextRemovalEndToEndTests
{
    ...
    public void the_recorded_event_list_carries_the_end_moment()
    {
        ...
        string source =
            ...
            + ">>\n"
            // Put the real procedure back BEFORE the check. `set!` on an imported binding
            // mutates the shared (lily) module variable, which outlives this run (as it does
            // in upstream LilyPond), while `orig` lives in this file's parser module and is
            // discarded with it: left in place, the wrapper would break the next \autoChange
            // in the process with "Unbound variable: orig" — which is exactly what happened
            // to autochange_music_engraves_to_a_page whenever xunit ran this test first.
            + "#(set! recording-group-emulate orig)\n"
