...
/// <para>
/// THIS IS THE CORRECTNESS CLAIM FOR THAT MECHANISM. The six regression pages that draw
/// a skyline (<c>show-skylines</c> and the five <c>skyline-*</c> files) cannot be graded
/// against the oracle under the port's OWN Emmentaler, because the port builds that font
/// from the Metafont sources on purpose and the build is not outline-identical. So the
/// claim lives here, where the expectation comes from upstream's own expression rather
/// than from the port's output:
/// </para>
/// <para>
/// //was previously: this paragraph said the six pages "are graded against a committed
/// PORT-GENERATED baseline instead", and that after ruling R9 these tests were "the only
/// thing carrying" the claim. Ruling R20 replaced that arrangement: the corpus is run
/// against LILYPOND'S OWN release fonts through <c>BatchDriver --fonts</c>, where the
/// port's answer must be exact, and the port's own font build is priced separately in
/// <c>font-delta-ledger.tsv</c>. R9's committed baseline DATA is gone. These tests stay
/// -- an oracle-backed corpus run and a hand-computed fence answer different questions --
/// but they are no longer the only thing carrying the claim:
/// </para>
/// <code>
...
public class GlyphOutlineSkylineTests
{
    ...
    public void a_glyphs_true_extent_is_read_from_its_outline_and_not_from_its_declared_box()
    {
        ...
        //
        // THIS is their correctness claim: that the quantity the corpus measures is read
        // from the OUTLINE. It has to be asserted separately, because
        // GlyphOutlineSkylineTests' other cases fence how many SEGMENTS the walk produces
        // and say nothing about which SOURCE the extent has.
        //was previously: "These two rows are graded against R9's port-generated baseline,
        // which claims NO DRIFT and nothing else (rule 33)." R20 replaced the baseline
        // with the run against LilyPond's own release fonts, and the baseline data is
        // gone; markup-with-true-dimensions is now the WORST row in the font-delta
        // ledger rather than a baseline row.
