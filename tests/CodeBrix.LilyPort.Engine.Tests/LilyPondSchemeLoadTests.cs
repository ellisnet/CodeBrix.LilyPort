...
public class LilyPondSchemeLoadTests
{
    ...
    // Kept so the ambient-publication fence can assert WHICH interpreter became
    // LilyPondScheme.Current, rather than merely that something did.
    private static Interpreter _loaded;
    ...
    /// <summary>
    /// WHERE THE AMBIENT INTERPRETER IS PUBLISHED — ruled 2026-08-27 (audit §2.9, open
    /// since 2026-08-03).
    /// <para>
    /// The two halves are one claim stated as a PAIR, because either alone is satisfiable
    /// by an accident: <c>CreateInterpreter</c> must NOT publish, and
    /// <c>LoadViaLilyScm</c> must. Asserting only the second would pass just as well if
    /// publication had stayed where it was, since the same interpreter comes out either
    /// way — the discriminator is that a bootstrapped interpreter which never loads the
    /// layer leaves the ambient ALONE.
    /// </para>
    /// <para>
    /// Why it matters, in the words the old publication site used without acting on them:
    /// a half-built interpreter must never become the ambient one. An interpreter with
    /// primitives and no Scheme layer has empty type-predicate tables, and anything that
    /// consults the ambient for a property type check in that window silently refuses
    /// every assignment.
    /// </para>
    /// </summary>
    [Fact]
    public void the_ambient_interpreter_is_published_by_the_layer_load_and_not_by_the_bootstrap()
    {
        //Arrange
        // Establishes a known ambient: the cached load's own interpreter.
        RunLoad(out _);
        LilyPondScheme.Current.Should().BeSameAs(_loaded);

        Interpreter ambientBefore = LilyPondScheme.Current;
        Interpreter bootstrapped = null;
        ...
        try
        {
            Interpreter.RunWithLargeStack(() =>
            {
                bootstrapped = LilyPondScheme.CreateInterpreter();
            });

            //Assert
            // THE DISCRIMINATOR: a bootstrapped interpreter is not ambient, and the one
            // that IS ambient is still the loaded one.
            bootstrapped.Should().NotBeNull();
            LilyPondScheme.Current.Should().NotBeSameAs(bootstrapped);
            LilyPondScheme.Current.Should().BeSameAs(ambientBefore);
        }
        finally
        {
