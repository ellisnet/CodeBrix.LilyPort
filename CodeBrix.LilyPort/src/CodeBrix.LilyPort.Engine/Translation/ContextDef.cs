...
/// definition.
/// <para>
/// A definition carries the context's name, its aliases, its translator
/// (<c>\consists</c>/<c>\remove</c>) list, its acceptance lists, its default property
/// operations and its description.
/// </para>
/// <para>
/// //was previously: this paragraph called the type what "the engine's interim
/// <c>Context.ContextFactory</c> seam stands in for", and said the integration halves
/// that read definitions back out of an output definition waited for the session that
/// retires that seam. The seam is retired and the member no longer exists; all three
/// halves are built — <see cref="PathToAcceptableContext"/> and
/// <see cref="PathToBottomContext"/> are reached from <c>Context</c>, and
/// <see cref="ApplyDefaultPropertyOperations"/> from <c>GlobalContext</c>.
/// </para>
/// </summary>
