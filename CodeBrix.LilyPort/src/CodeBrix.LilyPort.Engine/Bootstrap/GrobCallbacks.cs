...
public static class GrobCallbacks
{
    ...
    /// like over that range before line breaking has happened.
    /// //was previously: "These answer the ordinary extents — what a grob that declares
    /// no pure callback would give anyway; whether they should now route through the
    /// pure-property machinery (which has since landed) has not been re-measured." They
    /// DO route through it: <see cref="StencilIntegral.SimpleSkylinesFromExtents"/> reads
    /// Y through <c>MaybePureExtent</c> with the caller's pure/beg/end, exactly as
    /// upstream's <c>maybe_pure_internal_simple_skylines_from_extents</c> does, and the
    /// element-stencil pair carries them the same way. The claim outlived the fix that
    /// landed with the MeasureSpanner NaN defect, which StencilIntegral records at the
    /// site.
    /// </para>
    /// <para>
