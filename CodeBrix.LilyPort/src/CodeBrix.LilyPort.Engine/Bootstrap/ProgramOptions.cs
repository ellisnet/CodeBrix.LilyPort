...
namespace CodeBrix.LilyPort.Engine.Bootstrap; //was previously: lily/program-option.cc + lily/program-option-scheme.cc;
...
public sealed class ProgramOptions
{
    ...
    private readonly Dictionary<string, object> _declaredType
    ...
    /// <summary>
    /// Gets or sets the seam that applies a declared <c>#:type</c> PREDICATE to a
    /// candidate value — <c>(predicate, value)</c> answering whether the value satisfies
    /// it.
    /// <para>
    /// Upstream's <c>check_value_type</c> calls the predicate with <c>ly_call</c>
    /// (<c>lily/program-option-scheme.cc:456</c>), which needs the interpreter. The store
    /// is built before there is one, so the primitive layer installs this once when it
    /// binds the option primitives — the same arrangement <see cref="Output"/> has, and
    /// deliberately NOT a reach for the ambient <c>LilyPondScheme.Current</c>, which is
    /// still null while <c>lily.scm</c> is loading and calling <c>ly:set-option</c>.
    /// </para>
    /// </summary>
    public Func<object, object, bool> TypePredicateCheck { get; set; }
    ...
    /// <summary>
    /// Records an option's declared <c>#:type</c> VALUE — the predicate procedure, the
    /// list of allowed values, or one of the three type symbols — as opposed to
    /// <see cref="DeclareValueSyntax"/>, which records what that type means for reading
    /// <c>-d</c> TEXT.
    /// <para>
    /// Both come off the same keyword, and upstream keeps one thing where the port keeps
    /// two: the <c>program-option-type</c> object property is read for BOTH jobs, by
    /// <c>scm/lily.scm</c>'s command-line decode and by <c>check_value_type</c>. The port
    /// already mirrored the decode half into the store; this is the checking half, and it
    /// is mirrored for the same reason — so the store answers without a Scheme lookup.
    /// </para>
    /// </summary>
    /// <param name="name">The option name.</param>
    /// <param name="type">The declared type value.</param>
    public void DeclareType(string name, object type) => _declaredType[name] = type;

    /// <summary>
    /// Answers an option's declared <c>#:type</c> value, or <see langword="null"/> when
    /// nothing declared one.
    /// <para>
    /// Upstream's counterpart is <c>scm_object_property</c>, which answers <c>#f</c> for
    /// an option nobody declared — the arm <c>check_value_type</c> passes through
    /// unchecked, because a name the engine has never heard of is the user's own.
    /// </para>
    /// </summary>
    /// <param name="name">The option name.</param>
    /// <returns>The declared type, or <see langword="null"/>.</returns>
    public object DeclaredTypeOf(string name)
        => _declaredType.TryGetValue(name, out object type) ? type : null;

    /// <summary>
    /// Answers whether an option has been declared — upstream's
    /// <c>scm_hashq_get_handle</c> answering a handle rather than <c>#f</c>.
    /// </summary>
    /// <param name="name">The option name.</param>
    /// <returns><see langword="true"/> when the option exists in the store.</returns>
    public bool IsDeclared(string name) => _values.ContainsKey(name);

    /// <summary>
    /// Checks a candidate value against the option's declared <c>#:type</c>, warning and
    /// answering <see langword="false"/> when it does not fit —
    /// <c>lily/program-option-scheme.cc:455-491</c>'s <c>check_value_type</c>.
    /// <para>
    /// Two of the four declared shapes are checked and two are not, which is upstream's
    /// own arrangement: a PREDICATE is called, a LIST is searched with <c>equal?</c>, and
    /// the three type SYMBOLS (<c>string</c>, <c>string-or-boolean</c>,
    /// <c>string-or-false</c>) describe how to READ text rather than what a value may be,
    /// so they pass everything. So does an option nobody declared.
    /// </para>
    /// <para>
    /// The wording and the severity are upstream's, verbatim (rule 15), down to the
    /// doubled quotation marks a string value produces: upstream renders the value with
    /// <c>scm_object_to_string</c>, which WRITES it, and then wraps the result in quotes
    /// of its own — so <c>(ly:set-option 'resolution "abc")</c> reports
    /// <c>-dresolution=""abc""</c> and the command line's <c>-dresolution=abc</c>, whose
    /// text is READ into a symbol first, reports <c>-dresolution="abc"</c>. Both were
    /// measured against the pinned oracle.
    /// </para>
    /// </summary>
    /// <param name="name">The option name, after any <c>no-</c> prefix is stripped.</param>
    /// <param name="value">The candidate value.</param>
    /// <returns><see langword="true"/> when the value may be stored.</returns>
    public bool CheckValueType(string name, object value)
    {
        object type = DeclaredTypeOf(name);

        if (SchemeUtilities.IsProcedure(type))
        {
            if (TypePredicateCheck == null)
            {
                // Rule 4: a check that cannot run says so rather than answering yes.
                // The primitive layer installs the seam when it binds ly:add-option, so
                // reaching here means an option carried a predicate type into a store
                // that was never wired to an interpreter.
                throw new InvalidOperationException(
                    "no TypePredicateCheck is installed, so the declared type of option '"
                    + name + "' cannot be checked");
            }

            if (!TypePredicateCheck(type, value))
            {
                Flower.Warn.Warning(
                    "ignoring option -d" + name + "=\"" + Printer.Write(value)
                    + "\": value has wrong type");
                return false;
            }
        }
        else if (IsProperList(type))
        {
            if (!IsMember(value, type))
            {
                Flower.Warn.Warning(
                    "ignoring option -d" + name + "=\"" + Printer.Write(value)
                    + "\":\n  invalid value; possible values are " + Printer.Write(type));
                return false;
            }
        }
    ...
    /// <summary>Answers whether a value is a proper list — <c>ly_is_list</c>.</summary>
    /// <param name="value">The value to test.</param>
    /// <returns><see langword="true"/> for a proper list, the empty one included.</returns>
    ...
    /// <summary>
    /// Searches a list for a value with <c>equal?</c> — <c>scm_member</c>, reduced to the
    /// yes/no upstream's caller asks of it.
    /// </summary>
    /// <param name="value">The value to look for.</param>
    /// <param name="list">The list to search.</param>
    /// <returns><see langword="true"/> when the list holds the value.</returns>
    private static bool IsMember(object value, object list)
    {
        for (object cursor = list; cursor is Pair pair; cursor = pair.Cdr)
        {
            if (SchemeUtilities.IsEqual(value, pair.Car))
            {
                return true;
            }
        }
    ...
    public void SetFromOptionName(string name, object value)
    {
        if (name.StartsWith("no-", StringComparison.Ordinal))
        {
            ...
            // //was previously: `value = !IsTrue(value)', on the reading that Guile
            // counts only #f as false. That is what `scm_is_true' answers, and it is
            // NOT the conversion upstream uses here: `from_scm<bool>' defaults its
            // fallback to false, which makes it `scm_is_eq (s, SCM_BOOL_T)' --
            // EXACTLY #t and nothing else (lily/include/lily-guile.hh:468-479). The two
            // agree on every boolean and disagree on everything else, so
            // `(ly:set-option 'no-resolution 5)' negated to #f here where upstream
            // negates to #t. MEASURED against the pinned oracle, which reports
            // `ignoring option -dresolution="#t"'.
            value = !(value is bool boolean && boolean);
        }
        ...
        {
            // //was previously: reported through this store's own sink, which prints the
            // text bare. Upstream calls `warning' (lily/program-option-scheme.cc:377),
            // so the line carries the `warning: ' tag its severity earns -- which is
            // also what makes it visible to the diagnostics gate, and what lets
            // -dwarning-as-error act on it.
            Flower.Warn.Warning(
        ...
        // Upstream warns about a name nobody declared and then SETS IT ANYWAY
        // (lily/program-option-scheme.cc:385-387): the option is "probably used
        // privately by the user", so it is told about and kept. MEASURED: the oracle
        // answers `#t' for `(ly:get-option 'nosuchoption)' after warning about it, and
        // warns the same way for Frescobaldi's undeclared -ddebug-voices.
        if (!IsDeclared(name))
        {
            Flower.Warn.Warning("no such program option: " + name);
        }

        if (!CheckValueType(name, value))
        {
            return;
        }
    ...
    /// <summary>
    /// Adds a value to an accumulative option the way <c>ly:append-to-option</c> does —
    /// <c>lily/program-option-scheme.cc:493-519</c>.
    /// <para>
    /// The three tests happen in upstream's order, and the order is what decides how many
    /// diagnostics an undeclared name earns: the handle is looked up FIRST and returns,
    /// so <c>(ly:append-to-option 'nosuchoption 1)</c> reports only
    /// <c>no such program option</c> and never the accumulative complaint. MEASURED
    /// against the pinned oracle. //was previously: the primitive asked
    /// <see cref="IsAccumulative"/> first and <see cref="AppendTo"/> second, so an
    /// undeclared name collected BOTH warnings.
    /// </para>
    /// <para>
    /// Unlike <see cref="SetFromOptionName"/> this one does not strip a <c>no-</c>
    /// prefix — upstream does not either, and a negated accumulative option would mean
    /// nothing.
    /// </para>
    /// </summary>
    /// <param name="name">The option name.</param>
    /// <param name="value">The value to add.</param>
    public void AppendToOption(string name, object value)
    {
        if (!IsDeclared(name))
        {
            Flower.Warn.Warning("no such program option: " + name);
        ...
        // Upstream warns and STILL appends, which builds an improper list on an option
        // that never asked to accumulate -- see AppendTo, which reproduces the cons.
        if (!IsAccumulative(name))
        {
            Flower.Warn.Warning(
