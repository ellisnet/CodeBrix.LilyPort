...
public static class FontPrimitives
{
    ...
    /// <summary>
    /// The five entry points whose only consumer is the PostScript backend's
    /// font-embedding path — out of scope by a ratified decision (D15), and loud.
    /// <para>
    /// ⚠ AUDITED BY MECHANISM 2026-08-27, under D53's standing lesson: an N/A filed on a
    /// primitive's NAME is not filed at all. <c>ly:system-font-load</c> sat in this very
    /// method until its name was read as "load a font off the host" and its
    /// IMPLEMENTATION turned out to be a plain file search over the port's own assets, so
    /// each of these five was re-read the same way, in upstream's source rather than in
    /// its docstring.
    /// </para>
    /// <para>
    /// The finding: NONE of the five is a host lookup and none of them is beyond the
    /// port. They are file readers — three of them <c>FT_Face</c> reads over a font file
    /// (<c>lily/open-type-font-scheme.cc:127,174</c>, <c>lily/otf-scheme.cc:26</c>), one a
    /// raw <c>fopen</c> walk of a table directory (<c>:220</c>), one a raw copy of a face
    /// out of a collection (<c>:373</c>) — and the port's own <c>SfntReader</c> and
    /// <c>CffFont</c> already read every structure four of them report on. What makes
    /// them N/A is REACHABILITY, not capability: all eleven call sites in this whole tree
    /// are in the vendored <c>Scheme/lily/framework-ps.scm</c>, and D15 replaced that
    /// backend with reason. So each reason below names what the primitive DOES first and
    /// why nothing asks second, which is the shape that would have caught
    /// <c>ly:system-font-load</c>.
    /// </para>
    /// <para>
    /// Reversible by demand under D25, and cheaply: the day a consumer exists, four of
    /// the five are a short read over <c>SfntReader</c>. The fifth writes a FILE, which
    /// nothing in an SVG pipeline wants.
    /// </para>
    /// </summary>
    /// <param name="interpreter">The interpreter to install into.</param>
    private static void InstallNotApplicable(Interpreter interpreter)
    {
        NotApplicable(interpreter, "ly:otf->cff", 1, 2,
            "reads a font file's CFF table with FreeType and answers its bytes; the only "
            + "caller anywhere is framework-ps.scm's ps-embed-cff, and D15 replaced that "
            + "backend");

        NotApplicable(interpreter, "ly:get-cff-offset", 1, 2,
            "walks a font file's table directory for the byte offset of `CFF '; the only "
            + "caller anywhere is framework-ps.scm's subsetted-font embedding, and D15 "
            + "replaced that backend");

        NotApplicable(interpreter, "ly:has-glyph-names?", 1, 2,
            "answers FreeType's FT_HAS_GLYPH_NAMES for a font file; the only callers "
            + "anywhere are framework-ps.scm, choosing between a name-keyed and a "
            + "CID-keyed embedding, and D15 replaced that backend");

        NotApplicable(interpreter, "ly:get-font-format", 1, 2,
            "answers a font file's format as a symbol (CFF, TrueType, ...) from "
            + "FreeType; the only callers anywhere are framework-ps.scm, choosing an "
            + "embedding path, and D15 replaced that backend");

        NotApplicable(interpreter, "ly:extract-subfont-from-collection", 3, 3,
            "copies one face out of a `ttcf' collection into a new FILE; the only caller "
            + "anywhere is framework-ps.scm, D15 replaced that backend, and the port "
            + "ships no collection to take a face out of");
        ...
        interpreter.DefinePrimitive("ly:system-font-load", 1, 1, a =>
        {
            ...
            //was previously: this tested the result for null and raised the error
            // ITSELF, under a comment saying FindOtfFont "has already WARNED by this
            // point — that is its contract for the internal path, where a missing font
            // is recoverable". That contract was not real: nothing recovered a null,
            // and it reached ly:font-get-glyph as a wrong-type-arg (2026-08-26).
            // FindOtfFont now errors the way upstream's find_otf_font always has, so
            // the branch is unreachable and this primitive is once again
            // all-font-metrics-scheme.cc:46 exactly — a call and nothing else.
            return AllFontMetrics.FindOtfFont(name);
        });
    }
