...
public static class FontInterface
{
    ...
    public static FontMetric SelectFont(OutputDef layout, object chain)
    {
        ...
        if (ReferenceEquals(encoding, FetaBracesSymbol))
        {
            ...
            //was previously: a `braceFont == null ? null : ...' guard. It was the
            // port's own, and it existed only because FindOtfFont could answer null.
            // It errors instead now, as upstream's find_otf_font always did, so this
            // is once again font-select.cc:183-185 line for line.
            OpenTypeFontMetric braceFont = AllFontMetrics.FindOtfFont(name);
            return FindScaledFont(layout, braceFont, requestedSize / actualSize);
        }

        name += "-" + roundedSize.ToString(CultureInfo.InvariantCulture);

        //was previously: an `if (font == null) return null;' guard, the port's own and
        // now unreachable — see the brace branch above.
        ...
        double requestedSize)
    {
        ...
        // from step -24 to +24: quantizing here reproduces every one of them.
        //was previously: this note ended "... and formatting to three decimals in
        // TextFontMetric.DescriptionString reproduces every one of them". The QUANTIZING
        // half is right; the FORMATTING half was not, and the 31 samples did not reach a
        // size that could tell the difference. TextFontMetric.PangoSizeText carries the
        // measured format and the probe that measured it.
