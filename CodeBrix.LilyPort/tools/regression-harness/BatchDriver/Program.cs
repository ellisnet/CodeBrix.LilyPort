...
public static class Program
{
    ...
    private static void ClearStalePages(string outputDirectory)
    {
        ...
        // .midi files are written BY THIS DRIVER, beside the pages, so they are not
        // foreign and the clean's .svg-only scope leaves them alone on purpose (harness
        // README, "what the clean does not remove").
        //was previously: every remaining entry counted as foreign, so a normal sweep of
        // the regression corpus ended its clean with "66 non-.svg entr(ies) remain ... is
        // this the directory you meant?" every single time -- a permanent false alarm
        // over the driver's own output, which is the fastest way to teach a reader to
        // stop reading the line.
        int foreign = Directory.GetFileSystemEntries(outputDirectory)
            .Count(entry => !string.Equals(
                Path.GetExtension(entry), ".midi", StringComparison.OrdinalIgnoreCase));
        Console.WriteLine(string.Format(
            System.Globalization.CultureInfo.InvariantCulture,
        ...
        if (foreign > 0)
        {
            Console.WriteLine(string.Format(
                System.Globalization.CultureInfo.InvariantCulture,
                "# NOTE: {0} entr(ies) that are neither .svg nor .midi remain in the"
                + " output directory and were left alone — is this the directory you"
                + " meant?",
                foreign));
        }
    }
